...
class KPISeries(object):
    def __init__(self, value, timestamp, truth=None, label=None, missing=None):
        ...
        self._truth = np.asarray(truth, np.int64) if truth is not None else np.zeros(value.shape, dtype=np.int64)
        self._label = np.asarray(label, np.int64) if label is not None else np.zeros(value.shape, dtype=np.int64)
        self._missing = np.asarray(missing, np.int64) if missing is not None else np.zeros(value.shape, dtype=np.int64)
        ...
        if not np.max(intervals) == interval:
            ...
            new_label = np.zeros(new_timestamp.shape, dtype=np.int64)
            new_label[index] = self.label
            new_truth = np.zeros(new_timestamp.shape, dtype=np.int64)
            new_truth[index] = self.truth
            new_missing = np.ones(new_timestamp.shape, dtype=np.int64)
    ...
    @property
    def abnormal(self):
        return np.logical_or(self.missing, self.label).astype(np.int64)
    ...
    def split(self, radios):
        ...
        if np.asarray(radios).ndim == 1:
            ...
            split = np.asarray(np.cumsum(np.asarray(radios, np.float64)) * self.length, np.int64)
            split[-1] = self.length
...
class KpiFrameDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        normal = 1 - np.copy(self._strided_abnormal[item]).astype(np.int64)
        label = np.copy(self._strided_label[item]).astype(np.int64)
